...
class Mat3:
    def __init__(self, a=1,b=0,c=0,d=1,e=0,f=0):
        # SVG 2D affine matrix: [a c e; b d f; 0 0 1]
...
def parse_transform(txt: str) -> Mat3:
    ...
    while i < len(txt):
        ...
        # Note: rotate/skew not expected from dvisvgm for simple glyph placement.
        m = m @ comp
        i = k
    return m
...
def parse_svg(svg_path: Path) -> Tuple[Dict[str, str], List[dict], Dict[str, float]]:
    print(f"[INFO] Parsing SVG: {svg_path}")
    tree = etree.parse(str(svg_path))
    root = tree.getroot()
    ns = {"svg": "http://www.w3.org/2000/svg", "xlink": "http://www.w3.org/1999/xlink"}

    # Root viewBox (for reference / debugging)
    ...
    # Collect all <use> and compute final (x,y) by applying ancestor transforms + use@transform + (x,y) translation
    ...
    for idx, u in enumerate(uses):
        ...
        if not href or not href.startswith("#"):
            ...
            continue
        gid = href[1:]
        # start with identity, multiply ancestor transforms in document order
        M = Mat3.identity()
        for anc in u.iterancestors():
            t = anc.get("transform")
            if t:
                M = M @ parse_transform(t)
        # then the <use>'s own transform
        t = u.get("transform")
        if t:
            M = M @ parse_transform(t)
        x = float(u.get("x", "0"))
...
def render_glyph_png(gid: str, path_d: str, out_png: Path, raster_ppu: float, viewbox_units: float) -> Tuple[int,int,int,int]:
  ...
  <use xlink:href="#{gid}" x="0" y="0"/>
    ...
    # Using scale=raster_ppu gives: pixels per unit = raster_ppu (since width == viewBox width)
...
def extract_glyphs(svg_path: Path, out_dir: Path, manifest_path: Path, raster_ppu: float, viewbox_units: float) -> None:
    ...
    # For computing overall composition bounds in *SVG units* (root coords, y-down)
    min_x = math.inf
    min_y = math.inf
    max_x = -math.inf
    max_y = -math.inf
    ...
    for pl in placements:
        ...
        if gid not in glyph_png_cache:
            ...
            l, t, w, h = render_glyph_png(gid, path_lookup[gid], out_png, raster_ppu, viewbox_units)
            glyph_png_cache[gid] = {
                "file": out_png.name,
                "crop_left_px": l, "crop_top_px": t, "w_px": w, "h_px": h,
                # Convert to units using known raster_ppu
        ...
        # Update global bounds (in units): top-left = (X - crop_left_units, Y - crop_top_units)
        ...
        entries.append({
            ...
            "glyph_id": gid,
            "file": info["file"],
            # positions in *SVG units* (after transforms, y-down)
            "pos_units": [X, Y],
            # anchor to apply (so that top-left = pos_units - anchor_units)
            "anchor_units": [info["crop_left_units"], info["crop_top_units"]],
            # sizes both in units and pixels for convenience
    ...
    manifest = {
        ...
        "svg_meta": meta,
        # Pixels-per-unit used to rasterize glyph PNGs (decouples PNG resolution from on-screen size)
        "ppu": raster_ppu,
        # square viewBox width/height we used while rasterizing each glyph
        "glyph_viewbox_units": viewbox_units,
        # overall bounds of composed equation (in SVG units, y-down)
