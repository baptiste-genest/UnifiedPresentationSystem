                         ...
                         scalar height_ratio)
{
    ...
    std::string latex_cmd = "source ~/.bashrc ;source ~/.zshrc ;pdflatex -output-directory=/tmp /tmp/formula.tex  >>/tmp/UPS.log";
    if (std::system(latex_cmd.c_str())) {
        std::cerr << "[error while generating latex] " <<  std::endl;
        assert(false);
    }
    if (std::system("source ~/.bashrc ;source ~/.zshrc ;pdfcrop /tmp/formula.pdf  >>/tmp/UPS.log")){
        std::cerr << "[error while croping pdf] " << std::endl;
        assert(false);
    }
    if (std::system(("source ~/.bashrc ;source ~/.zshrc ; convert -density 800 -quality 100 /tmp/formula-crop.pdf -colorspace RGB " + filename + " >>/tmp/UPS.log").c_str())) {
        std::cerr << "[error while converting to png]" << std::endl;
        assert(false);
    }
    int h = 1080*height_ratio*Image::getSize(filename).y/99.;
    if (std::system(("source ~/.bashrc ;source ~/.zshrc ; convert " + filename + " -resize x" + std::to_string(h) + " " + filename + " >>/tmp/UPS.log").c_str())){
