...
void CreateMovingSlides(slope::Slideshow& show) {
    ...
    show << inNextFrame << PlaceRelative(Latex::Add("Suppose that you are at p $\\in$ M \\\\ and that you want to go in a direction v $\\in \\TM{p}$"),idea,slope::ABS_LEFT,slope::REL_BOTTOM,0.04,0.05);
    ...
    show << inNextFrame << approx->at(0.9);
    ...
    auto gamma = [v,n,x](scalar t,const TimeObject &time) {
        return vec(order2(x,v*t*0.7*smoothstep(time.from_action),1) + n*0.04);
    };
