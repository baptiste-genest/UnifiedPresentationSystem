...
Mat illustratePoissonProblem() {
    Mat B;
    std::string cache = Options::UPS_prefix + "cache/poisson.csv";
...
Mat EigenLaplace(const io::GeometryCentralMesh& mesh,std::string label,int k) {
    Mat Eig;
    std::string cache = Options::UPS_prefix + "cache/eig_"+label+ std::to_string(k) + ".csv";
...
Vec LapHeight(){
    Mat LH;
    std::string cache = Options::UPS_prefix + "cache/lap_height.csv";
    if (io::MatrixCache(cache,LH))
        return LH;


    io::GeometryCentralMesh mesh(Options::UPS_prefix + "meshes/mountain.obj");
    geometrycentral::surface::ExtrinsicGeometryInterface& geometry = *mesh.position_geometry;
    geometry.requireCotanLaplacian();
    geometry.requireVertexGalerkinMassMatrix();
    auto N = mesh.mesh->nVertices();
...
Mat Curvatures(const io::GeometryCentralMesh& mesh) {
    Mat curvature;
    std::string cache = Options::UPS_prefix + "cache/curvature.csv";
...
void init () {
    ...
    auto CMFONTID = UPS::FontManager::addFont(Options::UPS_prefix + "fonts/ComputerModernSR.ttf",50);
    UPS::Style::default_font = CMFONTID;

    auto grid = Mesh::Add(Options::UPS_prefix + "meshes/grid_quad_50.obj");
    auto disk = Mesh::Add(Options::UPS_prefix + "meshes/disk_coarse.obj",0.5,true);
    auto bunny = Mesh::Add(Options::UPS_prefix + "meshes/bunny.obj");
    auto bunny_coarse = Mesh::Add(Options::UPS_prefix + "meshes/bunny_coarse.obj");
    auto human = Mesh::Add(Options::UPS_prefix + "meshes/human.obj",0.2);
    auto mountain = Mesh::Add(Options::UPS_prefix + "meshes/mountain.obj",0.1);
    maskGC.init(Options::UPS_prefix + "meshes/nefertiti.obj");
    bunnyGC.init(Options::UPS_prefix + "meshes/bunny.obj");
    ...
    if (true)
    {
...

        {
            ...
            show << inNextFrame << PlaceRight(Image::Add(Options::UPS_prefix + "images/pavage_penta.png"),0.6,0.01);
    ...
    if (true)
    {
        ...
        show << PlaceLeft(Image::Add(Options::UPS_prefix + "images/simple_graph.png"),0.6,0.2);
        show << PlaceRelative(Image::Add(Options::UPS_prefix + "images/simple_lap_mat.png"),UPS::REL_RIGHT,SAME_Y);

        {
            ...
            auto diskverycoarse = Mesh::Add(Options::UPS_prefix + "meshes/disk_very_coarse.obj",1.3);
            ...
            show << newFrameSameTitle << PlaceBelow(Image::Add(Options::UPS_prefix + "images/cotan_angles.png"),0.1);
            show << PlaceBelow(Formula::Add("(Lf)_i =" + frac("1","A_i") + "\\sum_{(i,j)\\in E}"+frac("cot(\\alpha_{ij}) + cot(\\beta_{ij})","2")+"(f_i-f_j)"));
            show << inNextFrame << PlaceRight(Image::Add(Options::UPS_prefix + "images/cot_plot.png"));
        }
...

        {
...

            {
                ...
                auto mask = Mesh::Add(Options::UPS_prefix + "meshes/nefertiti.obj",0.5)->apply(offset(off),false);
                show << mask << top_cam;
...

            {
                ...
                show << PlaceLeft(Image::Add(Options::UPS_prefix + "images/poisson-g.png"),0.5,0.1);
                show << inNextFrame << PlaceRelative(Image::Add(Options::UPS_prefix + "images/poisson-u.png"),REL_RIGHT,SAME_Y,0.1);
                show << newFrameSameTitle << PlaceBelow(Latex::Add("Problème de Plateau (ou du film de savon)",0.04));
                show << PlaceBelow(Image::Add(Options::UPS_prefix + "images/plateau.png"));
                show << newFrameSameTitle << PlaceBelow(Latex::Add("Poisson Surface Reconstruction",0.04));
                show << PlaceBelow(Image::Add(Options::UPS_prefix + "images/PSR.jpg"));
                show << inNextFrame << PlaceBelow(Image::Add(Options::UPS_prefix + "images/PSR_algo.png"));
            }
...

            {
                ...
                auto humanGC = io::GeometryCentralMesh(Options::UPS_prefix + "meshes/human.obj");
                auto BCGC = io::GeometryCentralMesh(Options::UPS_prefix + "meshes/bunny_coarse.obj");
    ...
    if (true)
    {
        ...
        show << newFrameSameTitle;
        auto macaca = Mesh::Add(Options::UPS_prefix + "meshes/david.obj",0.01);
        io::GeometryCentralMesh mesh(Options::UPS_prefix + "meshes/david.obj");
        ...
        auto fan = Mesh::Add(Options::UPS_prefix + "meshes/fan.obj");
    ...
    if (true)
    {
        ...
        auto diskGC = io::GeometryCentralMesh(Options::UPS_prefix + "meshes/disk_coarse.obj");
        auto disk = Mesh::Add(Options::UPS_prefix + "meshes/disk_coarse.obj",1.,true);
        ...
        show << S << cam2 << Latex::Add("Divergence",0.05)->at(0.5,0.2) << AddPolyscopeQuantity(Qdiv) << PlaceRight(div_t,0.4,0.05) << PlaceBelow(Image::Add(Options::UPS_prefix + "images/fan_div.png"));
        ...
        show << S << cam2 << Latex::Add("Rotationnel",0.05)->at(0.5,0.2) << AddPolyscopeQuantity(Qcurl) << PlaceRight(curl_t,0.4,0.05) << PlaceBelow(Image::Add(Options::UPS_prefix + "images/fan_curl.png"));
